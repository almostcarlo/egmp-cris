...
class Applicant extends MY_Controller{
    ...
    /*public $doc_type = array('iprs picture'=>'Profile Picture','iprs cv'=>'Resume/CV', 'ppt'=>'Passport','nbi'=>'NBI Clearance', 'peos' => 'PEOS Certificate', 'ereg' => 'POEA E-Registration', 'coe'=>'Certificate of Employment','diploma_c'=>'College Diploma','diploma_h'=>'High School Diploma',
                            ...
                            'dr_lic'=>'Drivers License', 'oma' => 'OMA', 'data'=>'Data Flow', 'others'=>'Other Documents',);*/
    ...
    public function __construct(){
        ...
        $doctypes = getdata("select id,code,description from settings_doc_type");

        foreach($doctypes as $d){
        	$this->doc_type[$d['code']]['id'] = $d['id'];
        	$this->doc_type[$d['code']]['desc'] = $d['description'];
        }
    }
	...
	public function profile($id, $tab="personal"){
	    ...
	    /*GET BRANCHES*/
	    $data['branch_list'] = get_items_from_cache('branch');
	...
	public function ajax_tab(){
	    ...
	    switch($_GET['tab']){
	        ...
	        case 'emp_offer':
	            if($my_lineup_id <> '' && $my_lineup_id <> 0){
        	...
        	case 'processing':
	        	...
	        	$q_prin_poea = "";
	        	if($principal_id <> ''){
	        		$q_prin = " and principal_id = {$principal_id}";
	        		$q_prin_poea = " and mp.principal_id = {$principal_id}";
	        	}

	        	$q_lineup_id = "";
	        	if($my_lineup_info[0]['lineup_id'] <> ''){
	        		$q_lineup_id = "and ap.lineup_id = {$my_lineup_info[0]['lineup_id']}";
	        	}

	        	/*VISA*/
	        	$data['visa_list'] = getdata_for_dd("select id,visa_no as value from manager_visa where 1 {$q_prin} and expiry_date > '".date("Y-m-d",strtotime("today"))."'");
	...
	public function create_applicant(){
		$data['region_list'] = getdata_for_dd("select id, name as value from settings_region");
		//$data['province_list'] = getdata_for_dd("select id, name as value from settings_province");
		// $data['province_list'] = getdata_for_dd("select pr.id, pr.name as value
		// 										from settings_province pr
		// 										left join settings_city c
		// 										on pr.name = c.province
		// 										where 1
		// 										and c.name is not null
		// 										group by pr.id
		// 										order by pr.name asc");

		$data['create_error'] = false;
	    $this->template->set('file_javascript', array('javascripts/validation/create_applicant.js?version=1',));
	    $this->template->view('applicant/create', $data);
	}
	...
	public function ajax_get_province(){
		$province_list = getdata_for_dd("select id, name as value
										from settings_province
										where region_id = {$_GET['region_id']}
										order by name asc");
		echo generate_dd($province_list, '');
	}

	public function ajax_get_city(){
		$city_list = getdata_for_dd("select id, name as value
									from settings_city
									where province = {$_GET['province_id']}
									order by name asc");
		echo generate_dd($city_list, '');
	}
	...
	public function send_validation_email($applicant_id){
	    ...
	    if($app_info && valid_email($app_info[0]['email'])){
	        ...
	        if(VALIDATE_EMAIL_IN_RMS){
	        	$url = EXTERNAL_IP."/rs/home/validate_email/".base64_encode(date("Ymd", strtotime($app_info[0]['add_date'])).'-'.$applicant_id);
	        }else{
...
// 	    exit();
	...
	public function upload($what=NULL){
	    ...
	    if($what== 'profile_pix' && isset($_FILES['fileImage']) && $_FILES['fileImage']['size'] > 0){
	        ...
	        $current_picture = getdata("select * from applicant_uploads where applicant_id = {$_POST['textApplicant_id']} and type_id = {$type_id}");
	        ...
	        $config['max_size']            = MAX_FILE_UPLOAD;
	        $config['file_name']           = $_POST['selectDocType']."-".str_replace(" ", "-", pathinfo($_FILES['fileUpload']['name'], PATHINFO_FILENAME));
	        $file_input_name               = 'fileUpload';
	        $file_desc                     = $this->doc_type[$_POST['selectDocType']]['desc'];
	        $type_id					   = $this->doc_type[$_POST['selectDocType']]['id'];
	    }
	    ...
	    if($file_input_name <> ''){
	        ...
	        else
	        {
	            ...
	            /* SAVE TO DB */
	            if($this->applicant_model->save_uploads($config['upload_path'].$data['upload_data']['file_name'], $file_desc, $type_id)){
	        ...
	        if($record_id <> 0){
	            ...
	            if($lineup){
	                foreach ($lineup as $l_info){
	                    ...
	                    array_push($mr_pos_id, $l_info['mr_pos_id']);
	                }
...
// 	    var_dump($xxx);
	...
	public function download_file($id){
		...
		if(is_numeric($id)){
			...
			$fileinfo = getdata("select * from applicant_uploads where id={$id}");

			//$file = PORTAL_URL.$fileinfo[0]['filename'];
			$file = PORTAL_UPLOADS_FOLDER.substr($fileinfo[0]['filename'], 10);
			$newfile = basename($fileinfo[0]['filename']);
			$upload_path = "./uploads/applicant/".$fileinfo[0]['applicant_id']."/";

			/* CHECK IF UPLOAD FOLDER EXIST */
		    if(!is_dir($upload_path)) mkdir($upload_path, 0777, TRUE);

			if (!copy($file, $upload_path.$newfile)) {
				$this->session->set_flashdata('settings_notification_status', 'Error');
				$this->session->set_flashdata('settings_notification', 'Unable to download file.');
			}else{
				$this->session->set_flashdata('settings_notification_status', 'Success');
				$this->session->set_flashdata('settings_notification', 'File has been downloaded.');
			}

			// redirect('profile/'.$fileinfo[0]['applicant_id']."/overview", 'refresh');
			redirect($this->agent->referrer());
		}else{
			$this->session->set_flashdata('settings_notification_status', 'Error');
			$this->session->set_flashdata('settings_notification', 'Invalid ID.');

			redirect(BASE_URL."applicant/", 'refresh');
		}
	}
}
